...
# Shell-quote a value for safe inclusion in the sourced config file.
# Single-quotes the value and escapes embedded single quotes, so passwords
# containing spaces, $, !, quotes etc. survive bash sourcing intact.
sq() { printf "'%s'" "${1//\'/\'\\\'\'}"; }
...
# Detect the panel user (config files must be readable by the kiosk session,
# which runs as this user — root-owned 600 files break startup.sh sourcing)
PANEL_USER="$(logname 2>/dev/null || echo "${SUDO_USER:-}")"
if [[ -z "${PANEL_USER}" || "${PANEL_USER}" == "root" ]]; then
  # Fall back to the owner of the panel base directory
  PANEL_USER="$(stat -c '%U' "${PANEL_BASE}" 2>/dev/null || echo root)"
fi
...
# ── Resolve HA URL hostname → IP for MQTT default ─────────────────────────────
# paho-mqtt can resolve hostnames but homeassistant.local is mDNS — not
# reliable in all network configs. A raw IP is unambiguously safe.
# Uses Python (already a dependency) to mirror what the provisioning UI does.

HA_HOSTNAME="$(python3 -c "from urllib.parse import urlparse; print(urlparse('${NEW_HA_URL}').hostname)" 2>/dev/null || true)"
SUGGESTED_MQTT_HOST="${CURRENT_MQTT_HOST}"

if [[ -n "${HA_HOSTNAME}" ]]; then
  echo
  printf "  Resolving %s..." "${HA_HOSTNAME}"
  RESOLVED="$(python3 -c "
import socket, sys
try:
    ip = socket.getaddrinfo('${HA_HOSTNAME}', None, socket.AF_INET)[0][4][0]
    print(ip)
except Exception:
    sys.exit(1)
" 2>/dev/null || true)"
  ...
  if [[ -n "${RESOLVED}" ]]; then
    ...
    SUGGESTED_MQTT_HOST="${RESOLVED}"
  else
    printf " \033[33mcould not resolve — enter IP manually\033[0m\n"
    SUGGESTED_MQTT_HOST="${HA_HOSTNAME}"
  fi
fi
...
echo
prompt_val "MQTT Host / IP" "${SUGGESTED_MQTT_HOST}" NEW_MQTT_HOST "validate_host"
prompt_val "MQTT Port"      "${CURRENT_MQTT_PORT}" NEW_MQTT_PORT "validate_port"

heading "  Step 3 of 4 — MQTT Credentials"
echo -e "  ${DIM}Use a broker login (e.g. a Mosquitto add-on static login),${RESET}"
echo -e "  ${DIM}e.g. 'mqtt-panels'. Avoid spaces in the username.${RESET}"
echo
...
# All values are single-quoted (sq) so spaces and special characters in
# usernames/passwords survive bash sourcing.
...
#
PANEL_ID=$(sq "${NEW_PANEL_ID}")
HA_URL=$(sq "${NEW_HA_URL}")
MQTT_HOST=$(sq "${NEW_MQTT_HOST}")
MQTT_PORT=$(sq "${NEW_MQTT_PORT}")
MQTT_USER=$(sq "${NEW_MQTT_USER}")
MQTT_PASS=$(sq "${NEW_MQTT_PASS}")
BACKLIGHT_PATH=$(sq "${BACKLIGHT_PATH}")
ROTATION=90
EOF

chmod 600 "${CONFIG_FILE}"
chown "${PANEL_USER}:${PANEL_USER}" "${CONFIG_FILE}"
success "Wrote ${CONFIG_FILE} (owner: ${PANEL_USER})"

# ── Write /opt/ha-panel/sensor-config.py ──────────────────────────────────────
# Python string values: escape backslashes and double quotes.

py_esc() { local v="${1//\\/\\\\}"; printf '%s' "${v//\"/\\\"}"; }
...
PANEL_ID = "$(py_esc "${NEW_PANEL_ID}")"

# MQTT
MQTT_BROKER   = "$(py_esc "${NEW_MQTT_HOST}")"
MQTT_PORT     = ${NEW_MQTT_PORT}
MQTT_USERNAME = "$(py_esc "${NEW_MQTT_USER}")"
MQTT_PASSWORD = "$(py_esc "${NEW_MQTT_PASS}")"
MQTT_TOPIC_ROOT = f"home/{PANEL_ID}"

# I2C bus (bus 1 only - bus 0 does not exist on Pi 5)
I2C_BUS = 1
...
# Backlight
BACKLIGHT_PATH = "$(py_esc "${BACKLIGHT_PATH}")"
BACKLIGHT_MAX = 255
BACKLIGHT_ON = 255
BACKLIGHT_OFF = 0
...
chown "${PANEL_USER}:${PANEL_USER}" "${SENSOR_CONFIG_CANON}"
success "Wrote ${SENSOR_CONFIG_CANON} (owner: ${PANEL_USER})"
...
if [[ -d "${REPO_DIR}/sensor-daemon" ]]; then
  ...
  chown -h "${PANEL_USER}:${PANEL_USER}" "${SENSOR_CONFIG_LINK}" 2>/dev/null || true
