...
def sh_quote(value):
    """
    Single-quote a value for the sourced shell config file, escaping embedded
    single quotes. Passwords/usernames with spaces, $, ! etc. survive bash
    sourcing intact. (Unquoted values with spaces break `source` entirely.)
    """
...
def chown_to_panel_user(path):
    """
    Match the file's ownership to the panel base directory's owner so the
    kiosk session (running as the panel user) can read it. Root-owned 600
    config files break startup.sh sourcing. No-op if not running as root.
    """
    try:
        st = PANEL_BASE.stat()
        os.chown(path, st.st_uid, st.st_gid)
    except (OSError, PermissionError) as e:
        log.warning(f"chown {path} failed (non-fatal): {e}")
...
class ProvisioningHandler(BaseHTTPRequestHandler):
    ...
    def handle_configure(self):
        ...
        try:
            ...
            # All values single-quoted via sh_quote so spaces/special chars
            # in usernames or passwords survive bash sourcing.
            ...
            shell_config = (
                ...
                "#\n"
                f"PANEL_ID={sh_quote(panel_id)}\n"
                f"HA_URL={sh_quote(ha_url)}\n"
                f"MQTT_HOST={sh_quote(mqtt_host)}\n"
                f"MQTT_PORT={sh_quote(mqtt_port)}\n"
                f"MQTT_USER={sh_quote(mqtt_user)}\n"
                f"MQTT_PASS={sh_quote(mqtt_pass)}\n"
                f"BACKLIGHT_PATH={sh_quote(backlight_path)}\n"
                "ROTATION=90\n"
            )
            CONFIG_FILE.write_text(shell_config)
            os.chmod(CONFIG_FILE, 0o600)
            chown_to_panel_user(CONFIG_FILE)
            ...
            py_config = (
                ...
                "\n"
                f'PANEL_ID = "{py_quote(panel_id)}"\n'
                "\n"
                "# MQTT\n"
                f'MQTT_BROKER   = "{py_quote(mqtt_host)}"\n'
                f'MQTT_PORT     = {mqtt_port}\n'
                f'MQTT_USERNAME = "{py_quote(mqtt_user)}"\n'
                f'MQTT_PASSWORD = "{py_quote(mqtt_pass)}"\n'
                ...
                "\n"
                "# Backlight\n"
                f'BACKLIGHT_PATH = "{py_quote(backlight_path)}"\n'
                "BACKLIGHT_MAX = 255\n"
                "BACKLIGHT_ON = 255\n"
                "BACKLIGHT_OFF = 0\n"
                "\n"
            ...
            chown_to_panel_user(SENSOR_CONFIG_CANON)
